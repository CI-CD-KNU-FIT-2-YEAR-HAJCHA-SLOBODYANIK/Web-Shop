...
def setup_shop_data(db):
    ...
    p1 = Product.objects.create(
        category=cat_weapon, name='РДС-1', slug='rds-1',
        price=Decimal('230.00'), available=True
    )
    p2 = Product.objects.create(
        category=cat_weapon, name='Mark 15', slug='mark-15',
        price=Decimal('499.00'), available=True
    )
    p3 = Product.objects.create(
        category=cat_people, name='Іван', slug='ivan',
        price=Decimal('15.00'), available=False
    )
    p4 = Product.objects.create(
        category=cat_people, name='Петя', slug='petya',
        price=Decimal('20.00'), available=True
    )
    p5 = Product.objects.create(
        category=cat_tanks, name='Т-80', slug='t-80',
        price=Decimal('3200.00'), available=True
    )
    p6 = Product.objects.create(
        category=cat_tanks, name='Т-72', slug='t-72',
        price=Decimal('2600.00'), available=True
    )
    p7 = Product.objects.create(
        category=cat_tanks, name='Т-55', slug='t-55',
        price=Decimal('1200.00'), available=True
    )
...
def test_category_filtering(client, setup_shop_data):
    ...
    url = reverse('shop:product_list_by_category', args=[setup_shop_data['cat_people'].slug])
